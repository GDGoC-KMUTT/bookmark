        ...
        <div className="relative sm:w-[70%] xl:w-[700px] bg-white border-2 border-gray-300 shadow-sm rounded-lg p-10 flex flex-col space-y-3 mb-10">
            ...
            <ul className="space-y-3">
                {steps.length > 0 ? (
                    steps.map((step) =>
                        step.id && step.title ? (
                            <div key={step.id} onClick={() => (isEnrolled ? handleStepClick(step.id || 0) : null)}>
                                <StepCard stepId={step.id} title={step.title} check={step?.check ?? false} onSheetClose={fetchSteps} />
                            </div>
                        ) : null
                    )
                ) : (
